...
function App() {
  ...
  const analyzeRFP = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/analyze-rfp-multiple', formData, {
        headers: {
      ...
      if (err.response?.data?.error) {
        ...
        setError('Backend server is not running. Please start the backend service first.');
      } else {
  ...
  const analyzeAttachments = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/analyze-attachments', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      if (err.response?.data?.error) {
        ...
        setAttachmentError('Backend server is not running. Please start the backend service first.');
      } else {
  ...
  const generateRFP = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/generate-rfp', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      if (err.response?.data?.error) {
        ...
        setRfpError('Backend server is not running. Please start the backend service first.');
      } else {
  ...
  const generateDraftResponse = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/generate-draft-response', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      if (err.response?.data?.error) {
        ...
        setDraftError('Backend server is not running. Please start the backend service first.');
      } else {
  ...
  const isHeaderOrCertification = (text) => {
    const lowerText = text.toLowerCase();
    
    // Skip common headers and certifications that don't need responses
    return (
      lowerText.includes('affirmation of understanding') ||
      lowerText.includes('certification of compliance') ||
      lowerText.includes('state finance law') ||
      lowerText.includes('disclosure of prior non-responsibility') ||
      lowerText.includes('§139') ||
      lowerText.includes('requirements for completion') ||
      (lowerText.includes('offerer') && lowerText.includes('certification')) ||
      text.length > 80 // Very long text is likely a header
    );
  };

  const normalizeFieldName = (fieldName) => {
    // Normalize field names to detect duplicates
    const normalized = fieldName.toLowerCase()
      .replace(/[^\w\s]/g, '') // Remove punctuation
      .replace(/\s+/g, ' ') // Normalize whitespace
      .trim();
    
    // Map similar field types to common names
    if (normalized.includes('date')) return 'date';
    if (normalized.includes('name') && (normalized.includes('offerer') || normalized.includes('bidder') || normalized.includes('firm'))) return 'offerer_name';
    if (normalized.includes('authorized') && normalized.includes('representative')) return 'authorized_rep';
    if (normalized.includes('title')) return 'title';
    // Map both "address" and "contractor address" to the same field
    if (normalized.includes('address')) return 'contractor_address';
    if (normalized.includes('procurement') && normalized.includes('number')) return 'procurement_number';
    if (normalized.includes('governmental') && normalized.includes('entity')) return 'governmental_entity';
    if (normalized.includes('signature')) return 'signature';
    
    return normalized;
  };

  const determineFieldType = (description) => {
    const desc = description.toLowerCase();
    
    // Check for yes/no questions - including "(Please circle)" questions
    if ((desc.includes('yes') && desc.includes('no')) || 
        desc.includes('y/n') || 
        desc.includes('true/false') ||
        desc.includes('select yes or no') ||
        desc.includes('please circle') ||
        (desc.includes('?') && (desc.includes('yes') || desc.includes('no')))) return 'yesno';
    
    // Check for signature fields
    if (desc.includes('signature')) return 'signature';
    
    // Check for other field types
    if (desc.includes('checkbox') || desc.includes('check box')) return 'checkbox';
    if (desc.includes('date')) return 'date';
    if (desc.includes('number') || desc.includes('amount') || desc.includes('procurement')) return 'number';
